...
void loop() {
  ...
  byte r1 = analogRead(0);  // slows down the loop and for a better reading
  byte r2 = analogRead(0);
  //byte r3 = analogRead(0);
  read_bank[0]=analogRead(0);
...
//  Serial.println(read_bank[0]);
  ...
  int pot1t=readChange(0,analogRead(1));
...
if (pot1<16){
 ...
 //    snare_pitch= map (signal_stable,base_cutoff,255,20,250);
     snare_pitch=knee_map(signal_stable,16,271,30,280,171,60);
   ...
   if (signal_stable >=271 && signal_stable<521){
    ...
    kick_pitch=knee_map(signal_stable,271,521,40,330,421,100);
  ...
  if (signal_stable >=521 && signal_stable<771){
    ...
    tick_pitch=knee_map(signal_stable,521,771,30,330,672,100);
  ...
  if (signal_stable >=771){
    ...
    bass_pitch=knee_map(signal_stable,771,1023,50,900,921,250);
...
if (master_trigg==1){
       if (pot1>=16 && pot1<271){
        ...
        snare_pitch=knee_map(pot1,16,271,30,280,171,60);
      ...
      if (pot1>=271 && pot1<521){
        ...
        kick_pitch=knee_map(pot1,271,521,40,330,421,100);
      ...
      if (pot1>=521 && pot1<771){
        ...
        tick_pitch=knee_map(pot1,521,771,30,330,672,100);
        led_mode=3;
        accumulator3=0;
        B3_latch=1;
        master_trigg=0;  
      ...
      if (pot1>=771){
        ...
        bass_pitch=knee_map(pot1,771,1023,50,900,921,250);
        led_mode=4;  
        accumulator4=0;
        B4_latch=1;
        master_trigg=0;  
...
analogWrite(3, rled-modea); //red

//int inv_b=((bled)-255)*-1;
analogWrite(9, bled-modea); //blue

//int inv_g=-((gled)-255)*-1;
analogWrite(6, gled-modea); //green
